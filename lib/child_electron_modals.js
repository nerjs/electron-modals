...
class ChildElectronModals extends CloseMixin {
    constructor(name, props) {
        ...
        const beforeCloseWin = e => {
            this.close().catch(()=>{})
            e.returnValue = true
        }

        window.addEventListener('beforeunload', beforeCloseWin)
        this.on(CLOSE_PRIVATE_EVENT, () => {
            window.removeEventListener('beforeunload', beforeCloseWin)
        })
    }
}
